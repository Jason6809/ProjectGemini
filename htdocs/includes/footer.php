<?php echo "<script type='text/javascript' src='http://localhost:8000/includes/js/script.js'></script>"; ?>
<!-- <script type="text/javascript" src="../includes/js/script.js"></script> -->

<div class="footer">
	<div class="container">
		<div class="row">
			<div class="col-l-6 col-m-6 col-s-12">
				<div class="content">

					<p class="heading">About Us</p>

					<div class="card">
						<div class="content">
							<p>
								Vision
							<ul>
								<li>
									We aim to provide pleasant hospitality and professional services to welcome every guest who steps into the salon. A distinctive atmosphere to make each visit an enjoyable, pampering experience to look forward to every time.
								</li>
							</ul>
							</p>
						</div>


						<div class="content">
							<p>
								Philosophy
							<ul>
								<li>
									Every woman and man deserves to look great and feel confident. Number76 believes in creating more smiles in the world, starting with a hair style that complements your individual personality.
								</li>
							</ul>
							</p>
						</div>


						<div class="content">
							<p>
								Mission
							<ul>
								<li>
									To reach out to each customer's individual needs with the highest service standards.
								</li>

								<li>
									To inspire, nurture and educate the next generation on the charisma of hair stylists.
								</li>

								<li>
									To open up exciting opportunities for all pasisions in the beauty industry.
								</li>
							</ul>
							</p>
						</div>
					</div>
				</div>
			</div>


			<div class="col-l-6 col-m-6 col-s-12">
				<div class="content">

					<p class="heading">Contact Us</p>

					<div class="card">



						<div class="content">
							<p>
								Headquarter
							<ul>
								<li>Gemini Salon Mid Valley</li>
								<li>
									A-G-2, Ground Floor, Northpoint, Mid Valley City, No.1 Medan Syed Putra Utara, 59200 Kuala Lumpur.
								</li>
								<li>
									+603 2287 0661
								</li>
							</ul>
							</p>
						</div>

						<div class="content">
							<p>
								Customer Services
							<ul>
								<li>
									Mon to Friday 11:00 A.M until 9:00 P.M.
								</li>
								<li>
									Sat and Sun, No office.
								</li>
								<li>
									+603 2222 0000, +603 2222 1111
								</li>
								<li>
									gemini_salon@mail.com (24/7)
								</li>
							</ul>
							</p>
						</div>
					</div>
				</div>
			</div>
		</div>

		<div class="row">
			<div class="col-l-12 col-m-12 col-s-12">
				<div class="content center" style="border-top: 1px solid lightgray; padding: 15px 0;">
					<label>Copyright &copy; 2017 Gemini Hair Salon Sdn Bhd. All Rights Reserved.</label>
				</div>
			</div>
		</div>
	</div>
</div>
</body>

</html>